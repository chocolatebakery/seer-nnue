...
struct board {
  ...
  template <color c>
  [[nodiscard]] inline bool upcoming_cycle_exists_(const std::size_t& height, const board_history& history) const noexcept;

  [[nodiscard]] bool upcoming_cycle_exists(const std::size_t& height, const board_history& history) const noexcept;
  ...
  void feature_move_delta_(const move& mv, T0& feature_reset_cache, T1& sided_set) const {
    ...
    // In atomic chess, captures cause explosions that affect multiple pieces
    // Use full reset for captures to ensure NNUE stays synchronized
