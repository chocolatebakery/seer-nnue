...
class PerspectiveNetwork {
 private:
  ...
  using OutputStorageType = util::AlignedArray<util::simd::Alignment, typename Layer::OutputType, Layer::OutputCount>;
 ...
 public:
  ...
  [[nodiscard]] inline auto featureTransformer() const -> const auto & { return featureTransformer_; }
  ...
  inline auto propagate(const BitboardSet &bbs,
    ...
    std::get<0>(layers_).forward(bbs, stmInputs, nstmInputs, std::get<0>(storage));
  ...
  inline auto readFrom(IParamStream &stream) -> bool {
    return featureTransformer_.readFrom(stream) && readLayersFrom(std::make_index_sequence<sizeof...(Layers)>(), stream);
  }

  inline auto writeTo(IParamStream &stream) const -> bool {
    return featureTransformer_.writeTo(stream) && writeLayersTo(std::make_index_sequence<sizeof...(Layers)>(), stream);
  }
 ...
 private:
  ...
  inline auto forward(OutputStorage &storage, const BitboardSet &bbs) const {
    if constexpr (I > 0) {
      static_assert(std::tuple_element_t<I - 1, LayerStack>::OutputCount == std::tuple_element_t<I, LayerStack>::InputCount);
  ...
  inline auto propagate(OutputStorage &storage, const BitboardSet &bbs, std::index_sequence<Indices...>) const {
  ...
  inline auto readLayersFrom(std::index_sequence<Indices...>, IParamStream &stream) -> bool {
    bool success = true;
    ((success &= std::get<Indices>(layers_).readFrom(stream)), ...);
    return success;
  }
  ...
  inline auto writeLayersTo(std::index_sequence<Indices...>, IParamStream &stream) const -> bool {
    bool success = true;
    ((success &= std::get<Indices>(layers_).writeTo(stream)), ...);
    return success;
  }
