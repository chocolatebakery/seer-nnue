...
namespace eval::nnue {
template <typename Input, typename Param, activation::Activation Activation, u32 Inputs, u32 Outputs,
...
struct BaseAffineLayer {
  ...
  EVAL_SIMD_ALIGNAS std::array<ParamType, OutputBucketCount * WeightCount> weights;
  EVAL_SIMD_ALIGNAS std::array<ParamType, OutputBucketCount * BiasCount> biases;
...
template <typename Input, typename Param, activation::Activation Activation, u32 Inputs, u32 Outputs,
    output::OutputBucketing OutputBucketing>
inline auto operator>>(std::istream &stream, BaseAffineLayer<Input, Param, Activation, Inputs, Outputs, OutputBucketing> &layer)
    -> std::istream & {
...
template <typename Input, typename Param, activation::Activation Activation, u32 Inputs, u32 Outputs,
    output::OutputBucketing OutputBucketing>
inline auto operator<<(std::ostream &stream, const BaseAffineLayer<Input, Param, Activation, Inputs, Outputs, OutputBucketing> &layer)
    -> std::ostream & {
  return layer.writeTo(stream);
}

template <typename Input, typename Param, activation::Activation Activation, u32 Inputs, u32 Outputs,
    output::OutputBucketing OutputBucketing = output::Single>
struct DenseAffineLayer : BaseAffineLayer<Input, Param, Activation, Inputs, Outputs, OutputBucketing> {
  ...
  inline auto forward(const BitboardSet &bbs, std::span<const typename Base::InputType, Base::InputCount> inputs,
    ...
    for (u32 outputIdx = 0; outputIdx < Base::OutputCount; ++outputIdx) {
      ...
      for (u32 inputIdx = 0; inputIdx < Base::InputCount; inputIdx += ChunkSize) {
        ...
        const auto weightVec = util::simd::load<typename Base::ParamType>(&Base::weights[weightOffset + inputIdx]);
...
template <typename Input, typename Param, activation::Activation Activation, u32 Inputs, u32 Outputs,
    output::OutputBucketing OutputBucketing = output::Single>
struct DensePerspectiveAffineLayer : BaseAffineLayer<Input, Param, Activation, Inputs * 2, Outputs, OutputBucketing> {
  ...
  inline auto forward(const BitboardSet &bbs,
    ...
    for (u32 outputIdx = 0; outputIdx < Base::OutputCount; ++outputIdx) {
      ...
      for (u32 inputIdx = 0; inputIdx < PerspectiveInputCount; inputIdx += ChunkSize) {
        ...
        const auto weightVec = load<typename Base::ParamType>(&Base::weights[weightOffset + inputIdx]);
        const auto products = Activation::activateAndDot(inputVec, weightVec);
        sum = add<typename Base::OutputType>(sum, products);
      }
      ...
      for (u32 inputIdx = 0; inputIdx < PerspectiveInputCount; inputIdx += ChunkSize) {
        ...
        const auto weightVec = load<typename Base::ParamType>(&Base::weights[PerspectiveInputCount + weightOffset + inputIdx]);
        const auto products = Activation::activateAndDot(inputVec, weightVec);
        sum = add<typename Base::OutputType>(sum, products);
      }
