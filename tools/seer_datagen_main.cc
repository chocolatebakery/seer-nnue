...
struct options {
  std::string out_path{};
  std::string format{"marlinformat"};
...
void print_usage() {
  std::cout
      << "Usage:\n"
      << "  seer datagen --out PATH --format marlinformat --samples N [options]\n"
      << "  seer datagen gen --out PATH --format marlinformat --samples N [options]\n\n"
      << "Gen options:\n"
      << "  --out PATH                 Output .bin path (required).\n"
      << "  --format marlinformat       Output format (only marlinformat supported).\n"
...
int seer_datagen_gen_main(const int argc, const char* argv[]) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (starts_with(arg, "--format")) {
      ...
      opts.format = (*value == "marlin") ? "marlinformat" : *value;
      continue;
    }
  ...
  if (opts.format != "marlinformat") {
    std::cerr << "error: only --format marlinformat is supported\n";
    return 2;
  }
  if (opts.max_moves <= 0) {
  ...
  if (opts.use_startpos || (opts.epd_paths.empty() && !opts.use_startpos)) { seeds.push_back(train::state_type::start_pos()); }
...
int seer_datagen_main(const int argc, const char* argv[]) {
  if (argc >= 2) {
    ...
    if (sub == "rescore") {
      std::cerr << "error: rescore is not available for marlinformat output yet\n";
      return 2;
    }
  }
